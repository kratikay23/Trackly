...
function SignIn() {
    ...
    useEffect(() => {
        ...
        if (verified === "success") {
            ...
            toast.error("Account not found. Sign up again or contact support.");
        } else if (verified === "error") {
            toast.error("Verification failed. Try the link again or sign up.");
        }
    }, [searchParams]);
    ...
    const handleSubmit = async (event) => {
        try {
            ...
            const response = await apiClient.post(API.SIGN_IN, { email, password });
    ...
    const handleResendVerification = async () => {
        ...
        try {
            const res = await apiClient.post(API.RESEND_VERIFICATION, { email });
            toast.success(res.data?.message || "Verification email sent.");
            if (res.data?.verifyLink) {
                window.open(res.data.verifyLink, "_blank");
            }
        } catch (err) {
    ...
    const signupWithGoogle = async () => {
        try {
            ...
            if (!user) {
                toast.error("Google sign-in failed. No user info found.");
                return;
            }

            const token = await user.getIdToken();
            const res = await apiClient.post(API.GOOGLE_SIGN_IN, { token });

            if (res.data?.user) {
                const userData = { ...res.data.user, picture: user.photoURL };
            ...
            <div className="right-section d-flex flex-column justify-content-center align-items-center p-4">
                ...
                {isloading ? (
                    ...
                    <div className="form-box">
                        ...
                        <button
                            type="button"
                            className="btn auth-google-btn border"
